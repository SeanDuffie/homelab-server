...
if __name__ == "__main__":
    DOCKER_PATH = filedialog.askdirectory(initialdir=DEFAULT_PATH)
    WORLD_PATH = os.path.join(DOCKER_PATH, "data/world/")

    ############ GET ENVIRONMENT VARIABLES ############
    # ENV_DIR = os.path.join(os.path.dirname(__file__), ".env")
    ENV_DIR = os.path.join(DOCKER_PATH, ".env")
    ...
    if server_type is None:
        ...
        set_key(
            ...
            value_to_set=server_type
        )

    # eula()
    # gitignore()
    properties(world_path=WORLD_PATH,server_type=server_type)
    discordsrv(world_path=WORLD_PATH)
    essentialsx(world_path=WORLD_PATH)
